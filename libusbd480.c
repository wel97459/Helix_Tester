...
int libusbd480_GetDeviceDetails(TUSBD480 *di, char *name, uint32_t *width, uint32_t *height, uint32_t *version, char *serial)
{
	...
	if (result > 0)
	{
		...
		if (width)
			*width = (unsigned char)data[20] | ((unsigned char)data[21] << 8);
		if (height)
			*height = (unsigned char)data[22] | ((unsigned char)data[23] << 8);
		if (version)
			*version = (unsigned char)data[24] | ((unsigned char)data[25] << 8);
	}
	return result;
}
...
int process_inputReport(TUSBD480 *di, TUSBD480TOUCHCOORD16 *upos, TTOUCHCOORD *pos)
{
	...
	else if (pos->pen == 1 && di->touch.last.pen == 0)
	{ // pen up
		cleanQueue(&di->touch);
		return 3;
	}
	else if (pos->pen == 0 && di->touch.last.pen != 0)
	{ // pen down
		cleanQueue(&di->touch);
	}
